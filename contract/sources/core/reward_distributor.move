module flashbet::reward_distributor {
    ...
    public(package) fun add_stake(provider: address, amount: u64) acquires DistributionPool {
        ...
        let debt_adjustment_pos =
            (pool_ref.pool.acc_reward_per_share_positive * (amount as u128)) / PRECISION;
        let debt_adjustment_neg =
            (pool_ref.pool.acc_reward_per_share_negative * (amount as u128)) / PRECISION;
    ...
    fun get_pending_rewards_internal(
        pool_ref: &DistributionPool, provider: address
    ): (u128, u128) {
        if (!table::contains(&pool_ref.providers, provider))
            return (0, 0);
        ...
        let total_rewards_positive =
            ((provider_info.stake as u128) * pool_ref.pool.acc_reward_per_share_positive)
                / PRECISION;
        let total_rewards_negative =
            ((provider_info.stake as u128) * pool_ref.pool.acc_reward_per_share_negative)
                / PRECISION;

        let pending_positive =
            if (total_rewards_positive >= provider_info.reward_debt_positive) {
                total_rewards_positive - provider_info.reward_debt_positive
            } else { 0 };

        let pending_negative =
            if (total_rewards_negative >= provider_info.reward_debt_negative) {
    ...
    fun get_effective_balance_internal(
        pool_ref: &DistributionPool, provider: address
    ): u64 {
        if (!table::contains(&pool_ref.providers, provider))
            return 0;
        ...
        let (pending_positive, pending_negative) =
            get_pending_rewards_internal(pool_ref, provider);
    ...
    public fun remove_stake(provider: address, amount: u64): u64 acquires DistributionPool {
        ...
        let provider_reward_debt_pos =
            table::borrow(&pool_ref.providers, provider).reward_debt_positive;
        let provider_reward_debt_neg =
            table::borrow(&pool_ref.providers, provider).reward_debt_negative;

        // Calculate pending rewards
        let total_rewards_positive =
            ((provider_stake as u128) * pool_ref.pool.acc_reward_per_share_positive)
                / PRECISION;
        let total_rewards_negative =
            ((provider_stake as u128) * pool_ref.pool.acc_reward_per_share_negative)
                / PRECISION;

        let pending_rewards_pos =
            if (total_rewards_positive >= provider_reward_debt_pos) {
                total_rewards_positive - provider_reward_debt_pos
            } else { 0 };

        let pending_rewards_neg =
            if (total_rewards_negative >= provider_reward_debt_neg) {
        ...
        let effective_balance =
            if (effective_balance_u128 >= pending_rewards_neg) {
        ...
        let stake_to_remove =
            if (pending_rewards_pos >= pending_rewards_neg) {
    ...
    public(package) fun distribute_pnl(
        ...
        if (pnl_positive > 0) {
            let reward_per_share =
                (pnl_positive * PRECISION) / (pool_ref.pool.total_staked as u128);
            pool_ref.pool.acc_reward_per_share_positive += reward_per_share;
        };

        if (pnl_negative > 0) {
            let penalty_per_share =
                (pnl_negative * PRECISION) / (pool_ref.pool.total_staked as u128);
