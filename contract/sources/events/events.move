module flashbet::events {
    ...
    public(package) fun emit_placed_bet_event(
        bet_id: u64, user: address, amount: u64, expiry_time: u64
    ) {
        event::emit(
            PlacedBetEvent { bet_id, user, amount, expiry_time }
        );
    }
    ...
    public(package) fun emit_bet_resolved_event(
        bet_id: u64, resolver: address, won: bool, payout: u64
    ) {
        event::emit(
            BetResolvedEvent { bet_id, resolver, won, payout }
        );
    }
    ...
    public(package) fun emit_liquidity_added_event(
        provider: address, amount: u64
    ) {
        event::emit(LiquidityAdded { provider, amount });
    }
    ...
    public(package) fun emit_liquidity_removed_event(
        provider: address, amount: u64, remaining_stake: u64
    ) {
        event::emit(
            LiquidityRemoved { provider, amount, remaining_stake }
        );
    }
