...
module flashbet::test_flashbet {
    ...
    fun test_auto_cancel_on_late_resolution(
        admin: &signer,
        mock_usdc: &signer,
        aptos_framework: &signer,
        lp_provider: &signer,
    ...
    fun test_liquidity_management(
        ...
        assert!(
            liquidity_manager::get_total_liquidity() == lp_amount + add_amount,
            3
        );
        assert!(
            flashbet_core::get_vault_balance() == lp_amount + add_amount,
            3
        );
        ...
        assert!(
            liquidity_manager::get_total_liquidity()
                == lp_amount + add_amount - remove_amount,
            4
        );
        assert!(
            flashbet_core::get_vault_balance()
                == lp_amount + add_amount - remove_amount,
            4
        );
        ...
        let user1_store =
            primary_fungible_store::ensure_primary_store_exists(
                signer::address_of(user1), metadata
            );
